...
TEST(ServerTest, SpawnPlayerEntity) {
    ...
    reg.AddComponent(
        player, server::Component::Transform()
});
...
TEST(ServerTest, SpawnEnemyEntity) {
    ...
    reg.AddComponent(
        enemy, server::Component::Transform()
});
...
auto projectile = reg.SpawnEntity();
    reg.AddComponent(
        projectile, server::Component::Transform()
    });
    ...
    reg.AddComponent(
        entity, server::Component::Transform()
    });
    ...
    reg.AddComponent(
        stationary, server::Component::Transform()
    });
    ...
    TEST(ServerTcpTest, ConnectAckPacketRoundTrip) {
        for (uint8_t status_val = 0; status_val <= 3; ++status_val) {
            server::network::ConnectAckPacket original;
            original.player_id = server::network::PlayerId{
                static_cast<uint8_t>(status_val + 1)};
    ...
    TEST(ServerTcpEdgeCaseTest, PartialPayloadRead) {
        ...
        std::vector<uint8_t> partial(
            full_data.begin(), full_data.begin() + 22);
    ...
    TEST(ServerTcpEdgeCaseTest, ServerTrimsWhitespaceOnlyToEmpty) {
        ...
        std::string trimmed =
            (strBegin == std::string::npos)
                ? ""
                : username.substr(strBegin,
                      username.find_last_not_of(" \t") - strBegin + 1);
