 ...
 *
 * @param health_bar The HealthBarBoss component to initialize.
 */
void InitHealthBarBoss(Com::HealthBarBoss &health_bar) {
    if (!health_bar.foreground_texture.loadFromFile(
            "assets/images/ui/health_bar_boss/foreground_bar.png")) {
    ...
    if (!health_bar.green_texture.loadFromFile(
            "assets/images/ui/health_bar_boss/green_bar.png")) {
    ...
    if (!health_bar.yellow_texture.loadFromFile(
            "assets/images/ui/health_bar_boss/yellow_bar.png")) {
 ...
 *
 * @param health_bar The HealthBarBoss component to draw.
 * @param game_world The GameWorld providing the render window.
 */
void DrawHealthBarBoss(Com::HealthBarBoss &health_bar, GameWorld &game_world) {
 ...
 * @param health_bar The HealthBarBoss component to update.
 * @param game_world The GameWorld providing delta time.
 */
void UpdatePercentageHealthBarBoss(const Com::Health &health,
    Com::HealthBarBoss &health_bar, GameWorld &game_world) {
 ...
 * @param health_bar The HealthBarBoss component to update.
 */
void SetHealthsBarBossSprites(
    const Com::Transform &transform, Com::HealthBarBoss &health_bar) {
    ...
    sf::Vector2f posBar = sf::Vector2f(
        1920.f / 2.f, 1080.f - 50.f);  // Fixed position for boss health bar
...
void HealthBarBossSystem(Eng::registry &reg, GameWorld &game_world,
    ...
    for (auto &&[i, transform, health_bar, health] :
        ...
        if (!health_bar.is_loaded)
            InitHealthBarBoss(health_bar);
        if (!health_bar.is_loaded)
            continue;
        // Update Percentages Bars
        UpdatePercentageHealthBarBoss(health, health_bar, game_world);
        // Update health bar position
        SetHealthsBarBossSprites(transform, health_bar);
        DrawHealthBarBoss(health_bar, game_world);
    }
}
}  // namespace Rtype::Client
