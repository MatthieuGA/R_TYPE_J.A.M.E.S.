...
class registry {
 ...
 private:
    std::unordered_map <std::type_index, std::unique_ptr<components_holder_base>> _components_arrays;
    std::vector<std::function<void(registry&, Engine::entity const&)>> _erase_fns;
...
inline sparse_array<Component> &registry::register_component() {
  ...
  if (it == _components_arrays.end()) {
    ...
    std::function<void(registry&, entity const&)> fn = [](registry &r, entity const &e) {
      try {
  ...
  return static_cast<components_holder<Component>*>(_components_arrays[type_idx].get())->arr;
}
...
inline sparse_array<Component> const &registry::get_components() const {
  ...
  return static_cast<components_holder<Component> const *>(it->second.get())->arr;
}

template <typename Component>
inline typename sparse_array<Component>::reference_type registry::add_component(entity_t const &to, Component &&c) {
  return get_components<Component>().insert_at(to.getId(), std::forward<Component>(c));
}

template <typename Component, typename ... Params>
inline typename sparse_array<Component>::reference_type registry::emplace_component(entity_t const &to, Params &&... p) {
  return get_components<Component>().emplace_at(to.getId(), std::forward<Params>(p)...);
}
...
inline void registry::add_system(Function &&f) {
  auto wrapper = [func = std::forward<Function>(f)](registry &r) mutable {
    func(r, r.get_components<typename _unwrap_sparse_array<Components>::type>()...);
  };
