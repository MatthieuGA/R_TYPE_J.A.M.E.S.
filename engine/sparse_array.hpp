...
template <typename Component>
sparse_array<Component>::sparse_array(sparse_array const &other) : _data(other._data), _present(other._present) {}

template <typename Component>
sparse_array<Component>::sparse_array(sparse_array &&other) noexcept : _data(std::move(other._data)), _present(std::move(other._present)) {}
...
template <typename Component>
typename sparse_array<Component>::reference_type sparse_array<Component>::insert_at(size_type pos, Component &&comp) {
   return emplace_at(pos, std::move(comp));
}

template <typename Component>
typename sparse_array<Component>::reference_type sparse_array<Component>::insert_at(size_type pos, Component const &comp) {
...
template <typename Component>
typename sparse_array<Component>::size_type sparse_array<Component>::get_index(value_type const &val) const {
...
class zipper_iterator {
   ...
   using elem_t = decltype(std::declval<Container const>()[std::declval<std::size_t>()]);
...
public:
   ...
   zipper_iterator(iterator_tuple const &containers, std::size_t max, std::size_t idx = 0)
...
public:
   ...
   void incr_all(std::index_sequence<Is...>) {
      (void)std::initializer_list<int>{((void)++std::get<Is>(_containers), 0)...};
   }

   template <std::size_t... Is>
   bool all_set(std::index_sequence<Is...>) const {
      bool ok = true;
      (void)std::initializer_list<int>{((ok = ok && std::get<Is>(_containers)->has(_idx)), 0)...};
      return ok;
   }

   template <std::size_t... Is>
   value_type to_value(std::index_sequence<Is...>) const {
      return std::forward_as_tuple((std::get<Is>(_containers)->operator[](_idx))...);
   }
...
public:
   ...
   zipper(Containers const &... cs) : _begin(make_tuple_ptrs(cs...)), _size(_compute_size(cs...)) {}

   iterator begin() { iterator it(_begin, _size, 0); it.skip_to_valid(); return it; }
   iterator end() { return iterator(_begin, _size, _size); }

public:
   template <std::size_t... Is>
   static std::size_t _compute_size_impl(std::index_sequence<Is...>, Containers const &... cs) {
   ...
   template <class... C>
   static iterator_tuple _get_ptrs(C const &... cs) { return iterator_tuple(&cs...); }
...
public:
   using value_type = decltype(std::tuple_cat(std::tuple<std::size_t>{}, std::declval<typename base::value_type>()));

   indexed_zipper_iterator(typename base::iterator_tuple const &containers, std::size_t max, std::size_t idx = 0)
      : base(containers, max, idx) {}

   value_type operator*() const {
      return std::tuple_cat(std::tuple<std::size_t>(base::_idx), base::operator*());
   }
};
...
public:
   ...
   indexed_zipper(Containers const &... cs) : _zip(cs...) {}
   iterator begin() { auto it = iterator(_zip._begin, _zip._size, 0); it.skip_to_valid(); return it; }
   iterator end() { return iterator(_zip._begin, _zip._size, _zip._size); }
private:
