...
zipper_iterator<Containers...>::zipper_iterator(
    iterator_tuple containers, std::size_t max, std::size_t idx)
...
bool zipper_iterator<Containers...>::all_set(
...
zipper_iterator<Containers...>::to_value(std::index_sequence<Is...>) const {
    return std::forward_as_tuple(
        (std::get<Is>(_containers)->operator[](_idx).value())...);
}
...
template <class... Containers>
typename zipper<Containers...>::iterator zipper<Containers...>::begin() const {
    return iterator(_containers, _size, 0);
}

template <class... Containers>
typename zipper<Containers...>::iterator zipper<Containers...>::end() const {
