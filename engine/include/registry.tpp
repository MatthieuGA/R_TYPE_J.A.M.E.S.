...
inline sparse_array<Component> &registry::RegisterComponent() {
    ...
    return static_cast<components_holder<Component> *>(
        components_arrays_[type_idx].get())
        ->arr;
}
...
inline typename sparse_array<Component>::reference_type registry::AddComponent(
    entity_t const &to, Component &&c) {
    return GetComponents<Component>().insert_at(
        to.GetId(), std::forward<Component>(c));
}
...
template <typename T>
struct _unwrap_sparse_array {
    using type = T;
};

template <typename U>
struct _unwrap_sparse_array<sparse_array<U>> {
    using type = U;
};
