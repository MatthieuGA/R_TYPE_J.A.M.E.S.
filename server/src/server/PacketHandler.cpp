...
void PacketHandler::HandleSetDifficulty(
    ...
    // TODO(server-logic): Store difficulty setting and apply multipliers
    // to damage calculations and enemy fire rates in game systems
...
void PacketHandler::HandleSetKillableProjectiles(ClientConnection &client,
    ...
    // TODO(server-logic): Store killable projectiles setting and apply
    // to collision detection in projectile systems
    extern bool g_killable_enemy_projectiles;
    g_killable_enemy_projectiles = enabled;

    std::cout << "Killable enemy projectiles set to "
              << (enabled ? "ON" : "OFF") << " by player "
              << static_cast<int>(client.player_id_) << " ('"
              << client.username_ << "')" << std::endl;
