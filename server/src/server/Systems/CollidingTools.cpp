...
bool IsCollidingFromOffset(const Component::Transform &trans_a,
    ...
    vector2f scal_off_a = vector2f(off_a.x * scale_x_a, off_a.y * scale_y_a);
    vector2f scal_off_b = vector2f(off_b.x * scale_x_b, off_b.y * scale_y_b);
    vector2f scal_hb_a =
        vector2f(hb_a.width * scale_x_a, hb_a.height * scale_y_a);
    vector2f scal_hb_b =
        vector2f(hb_b.width * scale_x_b, hb_b.height * scale_y_b);
